...
class NAGAEMV(Optimizer):
    ...
    def __init__(self, lr=1, beta_1=0.9, beta_2=0.999, beta_3=0.999, momentum=0.95, decay=0.,
                 nesterov=True, **kwargs):
        super(NAGAEMV, self).__init__(**kwargs)
    ...
    def get_updates(self, loss, params):
        ...
        #whats = [K.zeros(shape) for shape in shapes]
        moments = [K.zeros(shape) for shape in shapes]
        self.weights = [self.iterations] + ws + ls + lhats + moments
        for p, g, w, l, lhat, m in zip(params, grads, ws, ls, lhats, moments):
            # w_t = g - l
            w_t = self.beta_1 * w + (1. - self.beta_1) * K.square(g)
            l_t = (K.sqrt(lhat)) / (K.sqrt(w_t)) * g
            lhat_t = self.beta_2 * lhat + (1. - self.beta_2) * K.square(l_t)
            #what_t = (self.beta_3 * what) + (1. - self.beta_3) * K.square(g)
            v = self.momentum * m - lr * l_t  # velocity
            self.updates.append(K.update(m, v))

            if self.nesterov:
                #new_p = p + self.momentum * v - lr * (l_t + K.pow((1 - self.beta_2), t) * K.sqrt(lhat_t)) / K.sqrt(what_t - K.square(l_t))
                new_p = p + self.momentum * v - lr * l_t
            else:
                new_p = p + v
    ...
    def get_config(self):
        ...
        base_config = super(NAGAEMV, self).get_config()
